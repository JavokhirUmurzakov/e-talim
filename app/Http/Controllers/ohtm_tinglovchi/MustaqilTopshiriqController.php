...
class MustaqilTopshiriqController extends Controller
{
    public function index($jurnal_id){
        ...
        $dars_kuns = Dars_kun::with('get_baho_only')->where('jurnal_id', $jurnal_id)
            ->where('nazorat_turi_id',4)->get();
        $bahos = Baho::orderBy('id')->get();
        $baho_array = $bahos->keyBy('id')->toArray();

        return view('ohtm_tinglovchi.mustqil_topshiriq', compact('jurnal', 'mavzus', 'dars_kuns', 'tinglovchis', 'bahos', 'baho_array'));
    }
}
